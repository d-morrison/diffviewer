...
diffviewer = (function() {
  ...
  diffviewer.init = function(el) {
    ...
    dv.render = function(message) {
      ...
      if(message.typediff == "text") {
        ...
        result = create_rds_diff($el, message.filename, message.diff_html);
  ...
  function create_rds_diff(el, filename, diff_html) {
    // For RDS files, the diff HTML is pre-generated by diffobj in R
    // We just need to wrap it in the appropriate structure and display it
    
    var state = {
      filename: filename,
      status: "modified"  // RDS diffs are always modifications (we wouldn't diff identical files)
    };
    ...
    var $wrapper = $(
      '<div class="rds-diff">' +
        '<div class="d2h-file-header">' +
          ...
          '</span>' +
        '</div>' +
        '<div class="rds-diff-container">' +
        '</div>' +
      '</div>'
    );

    $wrapper.find(".d2h-file-name").text(filename);
    $wrapper.find(".rds-diff-container").html(diff_html);

    // Clear and append
    $(el).html($wrapper);
    
    // Add diffviewer class for consistency
    el.addClass("diffviewer");
